<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <a href="/admin/companies" class="text-sm font-medium text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-2">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <form action="/admin/companies" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6 md:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="domain" value="<?= e(old('domain')) ?>" placeholder="example.com"
                            class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3"
                            class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"><?= e(old('description')) ?></textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Location</label>
                        <input type="text" name="location" value="<?= e(old('location')) ?>" placeholder="e.g. New York, USA"
                            class="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500">
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Avatar / Icon</label>
                ...
                <div class="pt-4 border-t border-gray-100">
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" name="is_public" value="1" checked class="mt-1 w-4 h-4 text-brand-600 rounded border-gray-300 focus:ring-brand-500">
                        <div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                <a href="/admin/companies" class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800 transition">Cancel</a>
...
<script>
    ...
    const existingLinks = <?= json_encode(old('social_platforms') ? array_map(function($p, $u, $i) { return ['platform' => $p, 'url' => $u, 'icon' => $i]; }, old('social_platforms'), old('social_urls', []), old('social_icons', [])) : []) ?>;
    if (existingLinks && existingLinks.length > 0) {
        existingLinks.forEach(link => {
            if(link.platform) addSocialLink(link.platform, link.url, link.icon);
        });
    }
</script>
